...
def crawl_and_scrape(url, level, max_level, sentences, main_url, visited_urls,depth_list): 
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            # Extract text from p tags and store complete sentences
            p_tags = soup.find_all('p')
            for p_tag in p_tags:
                text = p_tag.get_text()
                sentences.append(text)

            # Extract text from h1, h2, h3, h4, and h5 tags and store complete sentences
            h_tags = soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5'])
            for h_tag in h_tags:
                text = h_tag.get_text()
                sentences.append(text)

            # Extract text from ul and ol tags and store complete sentences
            ...
            # Extract text from div and table tags and store complete sentences
            div_tags = soup.find_all('div')
            table_tags = soup.find_all('table')
            for tag in div_tags + table_tags:
                text = tag.get_text()
                sentences.append(text)
...
for main_url in unique_domains:
    ...
    if sentences:
        ...
        output_file = f"./Scrapped_Past_Clients/{domain_name}.txt"
        ...
        file_path = "past_clients_status.xlsx"
